...
CLASSES = ['SDXL_Turbo', 'SDXL_Lightning', 'Flash_PixArt']
...
parser.add_argument('--image_path',default='/Scratch/Bertazzini/lab_images/SDXL_Turbo/image_5.png',type=str)
...
model = torch.load(f"attribution_models/clip_model.pth", map_location=device)
linear = torch.load(f"attribution_models/linear_model.pth", map_location=device)
...
with torch.no_grad():
    ...
    print(f"Image: {args.image_path}; Model: {CLASSES[predict[0]]}")