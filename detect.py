...
model = torch.load("detection_models/finetune_clip.pt").to(device)

linear = NeuralNet(1024,[512,256],2).to(device)
linear = torch.load('detection_models/clip_linear.pt')
...
with torch.no_grad():
    ...
    print(f"Image: {args.image_path} -> {CLASSES[predict[0]]}")
